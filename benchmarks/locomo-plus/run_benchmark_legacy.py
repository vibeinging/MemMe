...
def process_one(si, plus_item, context, total, config, run_path, done_idxs):
    ...
    try:
        ...
        store = memme.MemoryStore(
            ...
            enable_lattice=config.get("enable_lattice", False),
            enable_splade=config.get("enable_splade", False),
        ...
        if not skip:
            ...
            # Compact + meditate: create episode narratives + extract facts
            # Compact generates prospective queries that bridge semantic gaps
            for idx, sess in enumerate(context["sessions"]):
                try:
                    store.compact(f"s{idx}_{sess['type']}")
                except Exception:
                    pass
            try:
...
def main():
    ...
    p.add_argument("--enable-lattice", action="store_true", default=False)
    p.add_argument("--enable-splade", action="store_true", default=False)
    ...
    p.add_argument("--chat-api-key", default=None, help="Chat/Judge API key (defaults to --api-key)")
    ...
    chat_api_key = args.chat_api_key or args.api_key
    chat_providers = [{"api_key": chat_api_key, "base_url": args.chat_base_url}]
    ...
    config = dict(
        ...
        enable_lattice=args.enable_lattice, enable_splade=args.enable_splade,
