#!/usr/bin/env python3
"""
MemMe LOCOMO Benchmark v5 — Four-Layer Architecture with Meditation

Based on v3-fast, adds a meditation (consolidation) phase between ingestion and Q&A:
1. Identity Extraction: Extract stable personality traits from all facts
2. Fact Consolidation: Merge related facts, resolve conflicts
3. Cross-Entity Synthesis: Find shared facts between speakers

These additions directly address the single-hop and temporal/inference gaps.
"""
...
RULES:
- Base your answer on the memories provided below.
- For factual questions ("what/who/where/when/how many"), answer with ONLY what is explicitly stated in the memories. Use exact numbers, names, and details from the memories. Do not add items not mentioned.
- For opinion/inference questions ("would...?", "would...likely...?", "what personality...?"), you SHOULD reason and infer based on the person's known traits, preferences, and behaviors from the memories. Give your best judgment.
- When memories contain timestamps, more recent information takes precedence.
- Your answer should be concise (under 10 words when possible).
- Only say "I don't know" if there is truly NO relevant information in any memory.
...
class AsyncMemorySystem:
    ...
    async def _extract_facts(self, text: str, timestamp: str = "") -> list:
        ...
        try:
            ...
            print(f"      [WARN] Extraction failed: {str(e)[:60]}", flush=True)
            return []
...
async def ingest_conversation(config: BenchConfig, memory: AsyncMemorySystem, conv: dict):
    ...
    for num in session_nums:
        ...
        for i in range(0, len(messages_a), batch_size):
            ...
            all_tasks.append((batch_a, user_id_a, metadata))
            all_tasks.append((batch_b, user_id_b, metadata))

    # Process batches with controlled concurrency
    # We process sequentially per user to maintain dedup consistency,
    # but speaker A and speaker B can run in parallel
    tasks_a = [(msgs, uid, meta) for msgs, uid, meta in all_tasks if uid == user_id_a]
    tasks_b = [(msgs, uid, meta) for msgs, uid, meta in all_tasks if uid == user_id_b]

    async def process_user_batches(batches):
        for msgs, uid, meta in batches:
            try:
    ...
    await asyncio.gather(
        process_user_batches(tasks_a),
        process_user_batches(tasks_b),
    )

    return len(all_tasks), speaker_a, speaker_b, user_id_a, user_id_b
...
async def run_benchmark(config: BenchConfig):
    ...
    print(f"=== MemMe LOCOMO Benchmark v5 (Meditation) ===")
...
def main():
    parser = argparse.ArgumentParser(description="MemMe LOCOMO Benchmark v5 (Meditation)")
