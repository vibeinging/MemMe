#!/usr/bin/env python3
"""
MemMe LOCOMO Benchmark v3-fast — Async accelerated version

Same logic as v3 but with:
- asyncio + aiohttp for concurrent API calls
- Semaphore-based rate limiting (no sleeps)
- Parallel question answering
- Batch-style embedding with concurrency

~4-6x faster than sequential v3.
"""
...
async def run_benchmark(config: BenchConfig):
    ...
    print(f"=== MemMe LOCOMO Benchmark v3-fast (Async) ===")
    ...
    async with AsyncAPIClient(config) as client:
        ...
        for conv_idx, conv in enumerate(conversations):
            ...
            # Phase 2: Answer questions (in parallel batches)
...
def main():
    parser = argparse.ArgumentParser(description="MemMe LOCOMO Benchmark v3-fast (Async)")
