...
async def _llm_filter_memories(client: AsyncAPIClient, question: str, memories: list) -> list:
    """v8b: LLM rerank — reorder by relevance, keep ALL but prioritize relevant ones."""
    if not memories or len(memories) <= 5:
        return memories

    memories_text = "\n".join(f"{i+1}. {m}" for i, m in enumerate(memories))
    prompt = f"""Given this question, rank the memories by relevance. Return the memory numbers ordered from MOST relevant to LEAST relevant. Include ALL numbers.
...
Return a JSON array of ALL memory numbers ordered by relevance, e.g. [3, 1, 7, 2, ...].
Include EVERY number — do not skip any.
"""
    try:
        ...
        if isinstance(indices, list):
            # Reorder memories by LLM-ranked relevance, keep top-20
            reordered = []
            seen = set()
            for i in indices:
                if 1 <= i <= len(memories) and i not in seen:
                    reordered.append(memories[i-1])
                    seen.add(i)
...
async def answer_and_judge_question(
    ...
    try:
        ...
        # v8: LLM rerank — filter retrieved memories to only relevant ones
        raw_a = [r["content"] for r in results_a]
        raw_b = [r["content"] for r in results_b]
        filtered_a, filtered_b = await asyncio.gather(
            _llm_filter_memories(client, question, raw_a),
            _llm_filter_memories(client, question, raw_b),
        )

        memories_a = json.dumps(filtered_a, ensure_ascii=False)
        memories_b = json.dumps(filtered_b, ensure_ascii=False)
...
async def run_benchmark(config: BenchConfig):
    ...
    print(f"=== MemMe LOCOMO Benchmark v8 (LLM Rerank) ===")
...
def main():
    parser = argparse.ArgumentParser(description="MemMe LOCOMO Benchmark v8 (LLM Rerank)")
    ...
    parser.add_argument("--output-dir", default="results_v8")
