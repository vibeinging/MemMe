...
ANSWER_PROMPT = """You are answering a question based STRICTLY on the memories below. You must follow these rules precisely.

RULES:
1. ONLY use information that is EXPLICITLY stated in the memories. Do NOT infer, generalize, or add from your own knowledge.
2. For "what is X's status/identity" questions: Answer with the EXACT term from the memories (e.g. "Single", "Married", "Transgender woman").
3. For list questions ("what activities/books/events/items"): Scan ALL memories carefully. List EVERY matching item found, separated by commas. Do not omit any. Do not add any that aren't in the memories.
4. For "how many" questions: If an exact number is stated, use it. Otherwise count the distinct items explicitly mentioned.
5. For "where" questions: Give the specific place NAME (e.g. "Sweden", "beach, mountains, forest"), not a description.
6. For "who" questions: List ALL people/groups explicitly mentioned.
7. For opinion/inference questions ("would...?", "might...?"): Reason briefly based on known traits.
8. Answer with JUST the answer — no explanation, no sentences. Comma-separated list for multiple items.
9. Maximum 10 words for factual answers.
...
Return JSON: {"consolidated_facts": ["fact1", "fact2", ...]}"""
    ...
    def _entity_match_score(self, query_tokens: list, mem_content: str) -> float:
        """v9: Entity/keyword exact match scoring — finds ALL facts containing query entities."""
    ...
    def search(self, query_embedding: list, query_tokens: list,
               user_id: str, top_k: int = 30) -> list:
        """v9: Three-signal search — vector + BM25 + entity match."""
        user_memories = self.memories.get(user_id, [])
        if not user_memories:
            return []
        ...
        # Score every memory with 3 signals
        scored = []
        for m in user_memories:
            vec_sim = cosine_similarity(query_embedding, m["embedding"])
            bm25 = bm25_score(query_tokens, m["tokens"], N, avgdl)
            entity = self._entity_match_score(query_tokens, m["content"])
            scored.append((vec_sim, bm25, entity, m))

        # Normalize BM25 to 0-1 range
        max_bm25 = max((s[1] for s in scored), default=1.0) or 1.0

        # Three-signal RRF: rank by each signal, then fuse
        k = 60
        rrf_scores = defaultdict(float)
        content_map = {}

        # Signal 1: Vector similarity (weight 0.4)
        by_vec = sorted(scored, key=lambda x: x[0], reverse=True)
        for rank, (_, _, _, m) in enumerate(by_vec):
            key = id(m)
            rrf_scores[key] += 0.4 / (k + rank)
            content_map[key] = m

        # Signal 2: BM25 (weight 0.3)
        by_bm25 = sorted(scored, key=lambda x: x[1], reverse=True)
        for rank, (_, _, _, m) in enumerate(by_bm25):
            key = id(m)
            rrf_scores[key] += 0.3 / (k + rank)

        # Signal 3: Entity match (weight 0.3) — this is the new signal
        by_entity = sorted(scored, key=lambda x: x[2], reverse=True)
        for rank, (_, _, _, m) in enumerate(by_entity):
            key = id(m)
            rrf_scores[key] += 0.3 / (k + rank)
    ...
    async def search_multi_query(self, question: str, user_id: str, top_k: int = 30) -> list:
        """v9: Multi-query with three-signal retrieval."""
    ...
    def _generate_query_variants_local(self, question: str) -> list:
        """Generate query variants locally — v7: more variants for better list coverage."""
        variants = [question]
        ...
        # Variant 2: topic-specific rephrases
        q_lower = question.lower()
        if names:
            name = names[0]
            if 'activities' in q_lower or 'what does' in q_lower:
                variants.append(f"{name} hobbies interests activities enjoys does")
                variants.append(f"{name} hobby sport exercise routine")
            elif 'book' in q_lower or 'read' in q_lower:
                variants.append(f"{name} book reading title read")
                variants.append(f"{name} book novel story author")
            elif 'how many' in q_lower:
                variants.append(f"{name} {' '.join(key_terms[:3])} number count total")
            elif 'where' in q_lower:
                variants.append(f"{name} {' '.join(key_terms[:2])} location place")
                variants.append(f"{name} place visited location city country")
            elif 'event' in q_lower or 'participated' in q_lower or 'attended' in q_lower:
                variants.append(f"{name} event attended participated organized joined")
                variants.append(f"{name} event activity conference meeting")
            elif 'pet' in q_lower or 'animal' in q_lower or 'name' in q_lower:
                variants.append(f"{name} pet cat dog animal name")
            elif 'symbol' in q_lower or 'important' in q_lower:
                variants.append(f"{name} symbol important meaning special significant")
            elif 'support' in q_lower or 'help' in q_lower:
                variants.append(f"{name} support help mentor friend family")
            elif 'item' in q_lower or 'bought' in q_lower or 'purchase' in q_lower:
                variants.append(f"{name} bought purchased item shoes clothes gift")
            elif 'change' in q_lower or 'transition' in q_lower:
                variants.append(f"{name} change transition journey experience")
            elif 'what does' in q_lower and 'family' in q_lower:
                variants.append(f"{name} family together activity kids children")
            elif 'would' in q_lower:
                variants.append(f"{name} preference personality likes values character")
            else:
                if key_terms:
                    variants.append(f"{name} {' '.join(key_terms[:4])}")

        # Variant 3: just key terms (for broader matching)
        if key_terms and len(key_terms) >= 2:
            variants.append(' '.join(key_terms[:5]))

        return variants[:5]  # v7: up to 5 variants (was 3)
...
async def run_benchmark(config: BenchConfig):
    ...
    print(f"=== MemMe LOCOMO Benchmark v7 (Iter) ===")
...
def main():
    parser = argparse.ArgumentParser(description="MemMe LOCOMO Benchmark v7 (Iter)")
    ...
    parser.add_argument("--output-dir", default="results_v7")
