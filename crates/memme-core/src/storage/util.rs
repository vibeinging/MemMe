...
impl Storage {
    /// Ensure both online FTS indexes exist. They are created and backfilled
    /// once, then normal memory/event writes keep them synchronized.
    pub(crate) fn ensure_fts_indexes(&self) -> Result<()> {
        let memories_missing = self.backend.query_count(
            "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = $1",
            &[SqlParam::Text("memories_fts".to_string())],
        )? == 0;
        if memories_missing {
            self.create_fts_index()?;
        }

        let events_missing = self.backend.query_count(
            "SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = $1",
            &[SqlParam::Text("events_fts".to_string())],
        )? == 0;
        if events_missing {
            self.create_fts_index_events()?;
        }
        Ok(())
    }
    ...
    /// Delete source memories and every derived/indexed row in one transaction.
    ///
    /// `where_clause` is generated by internal callers only. When `history_user`
    /// is set, a DELETE history row is recorded before the transaction commits.
    pub(crate) fn delete_memories_where(
        &self,
        where_clause: &str,
        params: &[SqlParam],
        ...
        let vector_delete_sql = self.dialect().vector_delete_sql();

        self.backend.transaction(|tx| {
            let rows = tx.query_read(&select_sql, params, |row| {
                Ok((row.get_string(0)?, row.get_string(1)?))
            })?;
            let has_fts = tx.table_exists("memories_fts")?;

            for (memory_id, content) in &rows {
                let id_param = &[SqlParam::Text(memory_id.clone())];
                if let Some(sql) = vector_delete_sql {
                    tx.execute(sql, id_param)?;
                }
                if has_fts {
                    tx.execute("DELETE FROM memories_fts WHERE id = $1", id_param)?;
                }
                tx.execute(
                    "DELETE FROM associations WHERE from_id = $1 OR to_id = $1",
                    id_param,
                )?;
                tx.execute("DELETE FROM memory_entities WHERE memory_id = $1", id_param)?;

                if let Some(user_id) = history_user {
                    tx.execute(
                        "INSERT INTO history (id, memory_id, user_id, old_memory, new_memory, event) \
                         VALUES ($1, $2, $3, $4, $5, $6)",
                        &[
                            SqlParam::Text(uuid::Uuid::new_v4().to_string()),
                            SqlParam::Text(memory_id.clone()),
                            SqlParam::Text(user_id.to_string()),
            ...
            let affected = tx.execute(&delete_sql, params)?;
            if affected != rows.len() {
                return Err(MemoryError::Storage(format!(
                    "memory deletion changed during transaction: selected {}, deleted {affected}",
                    rows.len()
                )));
            }
            Ok(rows)
    ...
    pub(crate) fn prune_memories(
        ...
        let rows = self.delete_memories_where(
            &where_clause,
            &[SqlParam::Text(user_id.to_string())],
            None,
        )?;
    ...
    pub(crate) fn cleanup_expired(&self) -> Result<u64> {
        ...
        let where_clause = format!("expiration_date IS NOT NULL AND expiration_date < {now_ts}");
        Ok(self.delete_memories_where(&where_clause, &[], None)?.len() as u64)
    }
    ...
    pub(crate) fn consolidate(
        ...
        let deleted = if delete_below {
            ...
            self.delete_memories_where("user_id = $1 AND importance < $2", &del_params, None)?
                .len() as u64
        } else {
            0
        };
        ...
        let now_ts = self.dialect().current_timestamp_expr();
        let expired_where = format!("expiration_date IS NOT NULL AND expiration_date < {now_ts}");
        let expired_count = self.delete_memories_where(&expired_where, &[], None)?.len() as u64;
