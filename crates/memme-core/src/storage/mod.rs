...
/// Check whether the VSS extension is loaded in the current connection.
/// Returns `true` if HNSW index creation is available.
#[allow(dead_code)]
pub fn is_memme_db_available(conn: &Connection) -> bool {
    // Try a lightweight probe: create and drop a temp table with HNSW
    let result = conn.execute_batch(
        "CREATE TEMP TABLE __vss_probe (id INTEGER, v FLOAT[4]);
         INSERT INTO __vss_probe VALUES (1, [1.0, 0.0, 0.0, 0.0]);
         CREATE INDEX __vss_probe_idx ON __vss_probe USING HNSW (v) WITH (metric='cosine');
         DROP INDEX __vss_probe_idx;
         DROP TABLE __vss_probe;",
    );
    result.is_ok()
}
...
mod tests {
    ...
    fn test_memme_db_hnsw_index_e2e() {
        ...
        assert!(
            !indexes.is_empty(),
            "No HNSW found — VSS extension may not be loaded"
        );
    ...
    /// Test that HNSW index works with WHERE + ORDER BY + LIMIT (filtered search).
    /// Verifies the optimizer can handle the Filter node between Projection and Get.
    #[test]
    #[cfg(all(feature = "memme-db", not(feature = "bundled")))]
    fn test_memme_db_filtered_search() {
        let storage = open_storage(4);

        // Insert data for two different users
        let emb1 = vec![1.0, 0.0, 0.0, 0.0];
        let emb2 = vec![0.9, 0.1, 0.0, 0.0]; // close to emb1
        let emb3 = vec![0.0, 1.0, 0.0, 0.0]; // far from emb1

        storage
            .insert_memory("id1", "user1-hello", &emb1, "user1", "h1", &InsertMemoryParams::default())
            .unwrap();
        storage
            .insert_memory("id2", "user2-close", &emb2, "user2", "h2", &InsertMemoryParams::default())
            .unwrap();
        storage
            .insert_memory("id3", "user1-world", &emb3, "user1", "h3", &InsertMemoryParams::default())
            .unwrap();

        // Search with user_id filter — should only return user1's memories
        let results = storage
            .vector_search(&emb1, "user1", None, None, None, None, 10)
            .unwrap();
        assert_eq!(results.len(), 2, "Should only find user1's 2 memories");
        // id2 belongs to user2, should not appear
        assert!(
            results.iter().all(|r| r.user_id == "user1"),
            "All results should belong to user1"
        );

        // Direct SQL to test the optimizer pattern: WHERE + ORDER BY distance + LIMIT
        let conn = storage.read_conn();
        let query_vec = "[1.0, 0.0, 0.0, 0.0]::FLOAT[4]";
        let sql = format!(
            "SELECT id, user_id, array_cosine_distance(embedding, {}) AS distance \
             FROM memories \
             WHERE user_id = 'user1' \
        ...
        assert!(rows.iter().all(|(_, uid)| uid == "user1"));
    }
}
