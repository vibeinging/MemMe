...
unique_ptr<IndexScanState> HNSWIndex::InitializeFilteredScan(float *query_vector, idx_t limit, ClientContext &context,
                                                              const std::unordered_set<row_t> &valid_row_ids) {
	auto state = make_uniq<HNSWIndexScanState>();
	...
	// Use usearch's filtered_search with a predicate that checks the valid row_id set.
	// The predicate is called inline during HNSW graph traversal — all neighbors are
	// visited for graph connectivity, but only matching rows enter the top-k results.
	auto predicate = [&valid_row_ids](row_t key) -> bool {
		return valid_row_ids.count(key) > 0;
	};

	// Acquire a shared lock to search the index
	auto lock = rwlock.GetSharedLock();
	auto search_result = index.filtered_search(query_vector, limit, predicate);
